  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        ...
        <input type="date" value={filterDateFrom} onChange={(e) => setFilterDateFrom(e.target.value)} className="glass-input text-sm" />
        <input type="date" value={filterDateTo} onChange={(e) => setFilterDateTo(e.target.value)} className="glass-input text-sm" />
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-crm-border text-left text-crm-textMuted text-xs uppercase">
                  ...
                  {!isClient && <th className="p-4">Email</th>}
              ...
              <tbody>
                {leads.map((row) => (
                  ...
                  >
                    <td className="p-4 font-medium text-white">{row.name || row.leadName}</td>
                    <td className="p-4 text-crm-text">{row.phone || '—'}</td>
                    {!isClient && <td className="p-4 text-crm-text">{row.email || '—'}</td>}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="text-white font-medium">{detailLead.name}</p>
              </div>
              <div>
                <p className="text-crm-textMuted text-xs">Phone</p>
                <p className="text-white">{detailLead.phone || '—'}</p>
              </div>
              <div>
                <p className="text-crm-textMuted text-xs">Email</p>
                <p className="text-white">{detailLead.email || '—'}</p>
              </div>
              <div>
                <p className="text-crm-textMuted text-xs">Client</p>
                <p className="text-white">{detailLead.clientName}</p>
              </div>
              <div>
                <p className="text-crm-textMuted text-xs">Campaign</p>
                <p className="text-white">{detailLead.campaignName}</p>
              </div>
              <div>
                <p className="text-crm-textMuted text-xs">Source</p>
                <p className="text-white">{detailLead.leadSource || detailLead.source}</p>
              </div>
              <div>
                <p className="text-crm-textMuted text-xs">Created</p>
                <p className="text-white">{new Date(detailLead.date || detailLead.createdAt).toLocaleString()}</p>
              </div>
              <div>
