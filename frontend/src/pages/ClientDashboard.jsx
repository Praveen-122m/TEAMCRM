...
const ClientDashboard = () => {
  const { user, activeWorkspace, setActiveWorkspace, refreshUser } = useAuth();
  const navigate = useNavigate();
  const [workspaceInfo, setWorkspaceInfo] = useState(null);
  const [userReady, setUserReady] = useState(false);
  const workspaceId = user?.workspaces?.[0] || activeWorkspace;

  // On mount: always refresh user to get fresh clientProfileId from DB
  useEffect(() => {
    const init = async () => {
      if (user && !user.clientProfileId && user.role === 'Client') {
        await refreshUser();
      }
      setUserReady(true);
    };
  ...
  return (
    <div className="space-y-6">
      ...
      {workspaceId && userReady && user?.clientProfileId && (
        <div>
