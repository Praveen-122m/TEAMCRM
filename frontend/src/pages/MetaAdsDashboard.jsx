...
const MetaAdsDashboard = ({ embedded = false, workspaceId: propWorkspaceId, fixedClientId, clientLabel }) => {
  ...
  // Instagram Followers States
  const [followersLatest, setFollowersLatest] = useState(null);
  ...
  const loadHistoricalMetrics = useCallback(async () => {
    ...
    try {
      const [totalsRes, chartsRes, campaignsRes, latestRes, historyRes, chartRes] = await Promise.all([
        ...
        metaHistoryService.getFollowersLatest(selectedClient).catch(err => {
          console.error('Error fetching followers latest:', err);
          return { data: { latest: 0, weeklyGrowth: 0, monthlyGrowth: 0 } };
        }),
        metaHistoryService.getFollowersHistory(selectedClient, startStr, endStr).catch(err => {
          console.error('Error fetching followers history:', err);
          return { data: [] };
        }),
        metaHistoryService.getFollowersChart(selectedClient, startStr, endStr).catch(err => {
          console.error('Error fetching followers chart:', err);
          return { data: [] };
        })
      ]);
      ...
      setFollowersLatest(latestRes.data);
      setFollowersHistory(historyRes.data);
      setFollowersTimeline(chartRes.data);
      setCurrentPage(1); // reset to page 1 on filter change
      setFollowersPage(1); // reset followers page on filter change
    } catch (err) {
      ...
      setFollowersLatest(null);
      setFollowersHistory([]);
      setFollowersTimeline([]);
    } finally {
      setHistoricalLoading(false);
    }
  ...
  // Followers pagination logic
  const followersItemsPerPage = 10;
  const totalFollowersPages = Math.ceil(followersHistory.length / followersItemsPerPage);
  const indexOfLastFollower = followersPage * followersItemsPerPage;
  const indexOfFirstFollower = indexOfLastFollower - followersItemsPerPage;
  const currentFollowers = followersHistory.slice(indexOfFirstFollower, indexOfLastFollower);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            type="button" 
            onClick={handleSync} 
            className="glass-button flex items-center gap-2 py-2 px-4 text-sm font-semibold text-crm-text hover:text-crm-primary transition-all duration-300"
          >
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {dateRangeOption === 'custom' && (
              <div className="flex items-center gap-2 bg-crm-darker/40 p-1 rounded-lg border border-crm-primary/10">
                <input
                  ...
                  className="glass-input text-xs text-crm-text bg-crm-darker/90 py-1 px-2 max-w-[120px]"
                />
            ...
            <div className="flex items-center gap-1">
              <button 
                ...
                className="glass-button p-2 text-crm-textMuted hover:text-rose-400 border border-crm-primary/5 hover:border-rose-500/20"
              ...
              <button 
                ...
                className="glass-button p-2 text-crm-textMuted hover:text-emerald-400 border border-crm-primary/5 hover:border-emerald-500/20"
        ...
        {historicalLoading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="glass-panel p-5 relative overflow-hidden flex flex-col justify-between border border-crm-primary/10">
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <LinkIcon size={20} className="text-violet-500 opacity-60" />
                </div>
              </div>
              ...
              <div className="glass-panel p-5 relative overflow-hidden flex flex-col justify-between border border-crm-primary/10">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-crm-textMuted text-xs font-semibold uppercase tracking-wider">IG Followers</p>
                    <h3 className="text-2xl font-black text-crm-text mt-1">
                      {(followersLatest?.latest || 0).toLocaleString()}
                    </h3>
                  </div>
                  <Instagram size={20} className="text-pink-500 opacity-60" />
                </div>
                <div className="mt-4 pt-3 border-t border-crm-primary/5 grid grid-cols-2 gap-2">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-crm-textMuted uppercase font-semibold">Weekly Growth</span>
                    <span className={`text-xs font-bold flex items-center gap-0.5 mt-0.5 ${
                      (followersLatest?.weeklyGrowth || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'
                    }`}>
                      {(followersLatest?.weeklyGrowth || 0) >= 0 ? '↑' : '↓'} {Math.abs(followersLatest?.weeklyGrowth || 0).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-crm-textMuted uppercase font-semibold">Monthly Growth</span>
                    <span className={`text-xs font-bold flex items-center gap-0.5 mt-0.5 ${
                      (followersLatest?.monthlyGrowth || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'
                    }`}>
                      {(followersLatest?.monthlyGrowth || 0) >= 0 ? '↑' : '↓'} {Math.abs(followersLatest?.monthlyGrowth || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="glass-panel p-6 h-96 border border-crm-primary/5">
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-crm-text">Date-Wise Instagram Followers Graph</h3>
                <p className="text-xs text-crm-textMuted">Daily follower growth trend</p>
              </div>
              <div className="w-full h-[calc(100%-3.5rem)]">
                {followersTimeline.length === 0 ? (
                  <div className="flex h-full items-center justify-center text-crm-textMuted text-sm">No historical follower data available for this range.</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={followersTimeline} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      <defs>
                        <linearGradient id="colorHistoryFollowers" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '0.5rem', color: '#f8fafc' }} formatter={(v) => [v.toLocaleString(), 'Followers']} />
                      <Area type="monotone" dataKey="instagram_followers" stroke="#ec4899" strokeWidth={2.5} fillOpacity={1} fill="url(#colorHistoryFollowers)" />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
                ...
                <table className="min-w-full text-left text-xs">
                  <thead className="bg-crm-darker/60 uppercase font-semibold text-crm-textMuted border-b border-crm-primary/10">
                    <tr>
                      ...
                      <th className="py-3 px-4 text-right">Messaging</th>
                    </tr>
                  </thead>
                    ...
                    {currentCampaigns.length === 0 ? (
                      ...
                      currentCampaigns.map((row, idx) => (
                        <tr key={idx} className="hover:bg-crm-primary/5 transition-all duration-150">
                          ...
                          <td className="py-3 px-4 text-right text-cyan-400">{(row.messaging_conversations || 0).toLocaleString()}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
                ...
                <div className="flex justify-between items-center mt-6 pt-4 border-t border-crm-primary/5 text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="glass-button p-2 text-crm-textMuted hover:text-crm-primary disabled:opacity-50 disabled:hover:text-crm-textMuted"
                    >
                      <ChevronLeft size={14} />
                    </button>
            ...
            {/* Followers History Table */}
            <div className="glass-panel p-6 border border-crm-primary/5">
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-crm-text">Instagram Followers History Log</h3>
                <p className="text-xs text-crm-textMuted">Daily follower snapshot counts and growth differences.</p>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-xs">
                  <thead className="bg-crm-darker/60 uppercase font-semibold text-crm-textMuted border-b border-crm-primary/10">
                    <tr>
                      <th className="py-3 px-4">Date</th>
                      <th className="py-3 px-4 text-right">Followers Count</th>
                      <th className="py-3 px-4 text-right">Growth Difference</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-crm-primary/5">
                    {currentFollowers.length === 0 ? (
                      <tr>
                        <td colSpan="3" className="text-center py-8 text-crm-textMuted">No follower history records found for this range.</td>
                      </tr>
                    ) : (
                      currentFollowers.map((row, idx) => (
                        <tr key={idx} className="hover:bg-crm-primary/5 transition-all duration-150">
                          <td className="py-3 px-4 font-mono text-crm-textMuted">{row.date}</td>
                          <td className="py-3 px-4 text-right text-crm-text font-semibold">{(row.followers_count || 0).toLocaleString()}</td>
                          <td className={`py-3 px-4 text-right font-bold ${
                            (row.growth_difference || 0) > 0 ? 'text-emerald-400' : (row.growth_difference || 0) < 0 ? 'text-rose-400' : 'text-crm-textMuted'
                          }`}>
                            {(row.growth_difference || 0) > 0 ? '+' : ''}{(row.growth_difference || 0).toLocaleString()}
                          </td>
                        </tr>
                      ))
                    )}
              ...
              {totalFollowersPages > 1 && (
                <div className="flex justify-between items-center mt-6 pt-4 border-t border-crm-primary/5 text-xs">
                  <p className="text-crm-textMuted">
                    Showing <span className="font-semibold text-crm-text">{indexOfFirstFollower + 1}</span> to{' '}
                    <span className="font-semibold text-crm-text">
                      {Math.min(indexOfLastFollower, followersHistory.length)}
                    </span>{' '}
                    of <span className="font-semibold text-crm-text">{followersHistory.length}</span> records
                  </p>
                  
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setFollowersPage(prev => Math.max(prev - 1, 1))}
                    ...
                    <button
                      onClick={() => setFollowersPage(prev => Math.min(prev + 1, totalFollowersPages))}
...
const GrowthBadge = ({ value, label }) => {
  ...
  return (
    ...
    >
      {isPositive ? '↑' : '↓'} {absVal}% <span className="text-[9px] text-crm-textMuted font-normal ml-0.5">{label}</span>
    </span>
  );
};
