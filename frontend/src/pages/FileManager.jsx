...
import { Upload, FileText, Image as ImageIcon, Download, Trash2, Share2 } from 'lucide-react';
...
const FileManager = () => {
  ...
  useEffect(() => {
    ...
    const handleFileUploaded = (newFile) => {
      if (newFile.workspaceId?.toString() === workspaceId?.toString()) {
        setFiles((prev) => {
  ...
  const getFileIcon = (file) => {
    ...
    return <FileText className="text-crm-primary" size={24} />;
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-crm-text tracking-tight">File Manager</h1>
          <p className="text-crm-textMuted text-sm mt-1">Manage creatives, reports, and client documents.</p>
        </div>
        <div>
          <input type="file" ref={fileInputRef} onChange={handleUpload} className="hidden" accept="image/*,video/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.zip,.rar,.7z,.mp3,.wav,.m4a,.ogg,.aac" />
          {selectedFiles.length > 0 ? (
            <div className="flex gap-2">
              <button type="button" onClick={() => setSelectedFiles([])} className="glass-button-secondary">
                Cancel
              </button>
              <button type="button" onClick={() => setSharingFile(selectedFiles)} className="glass-button">
                <Share2 size={18} /> Share Selected ({selectedFiles.length})
              </button>
            </div>
          ) : (
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="glass-button disabled:opacity-50">
              <Upload size={18} /> {uploading ? `Uploading ${uploadProgress}%...` : 'Upload File'}
            </button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {files.map((file) => (
            <div key={file._id} className={`glass-card p-4 group relative ${selectedFiles.some(f => f._id === file._id) ? 'ring-2 ring-crm-primary' : ''}`}>
              <div className={`absolute top-2 left-2 z-20 bg-crm-card/90 backdrop-blur-md rounded-lg p-1.5 border border-crm-border shadow-sm flex items-center justify-center transition-opacity ${selectedFiles.some(f => f._id === file._id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                <input 
                  type="checkbox" 
                  checked={selectedFiles.some(f => f._id === file._id)}
                  onChange={(e) => {
                    if (e.target.checked) setSelectedFiles([...selectedFiles, file]);
                    else setSelectedFiles(selectedFiles.filter(f => f._id !== file._id));
                  }}
                  className="w-4 h-4 rounded border-crm-border text-crm-primary focus:ring-crm-primary bg-crm-darker cursor-pointer accent-crm-primary"
                />
              </div>
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2 z-10">
                <button type="button" onClick={() => setSharingFile([file])} className="p-1.5 bg-crm-darker/90 rounded hover:text-blue-400" title="Share">
                  <Share2 size={14} />
                </button>
              ...
              <p className="text-sm font-medium text-crm-text truncate mb-1" title={file.originalName || file.name}>
