...
const NotificationPopup = () => {
  ...
  const handleClick = () => {
    ...
    } else if (notification?.type?.startsWith('task_') || p.taskId || notification?.content?.toLowerCase().includes('task')) {
  ...
  return (
    <div className="fixed top-20 right-4 z-[100] max-w-sm animate-in slide-in-from-right">
      <button
        type="button"
        onClick={handleClick}
        className="w-full bg-crm-card border border-crm-border p-4 flex items-start gap-3 text-left hover:border-crm-primary/60 transition-colors shadow-lg rounded-2xl"
      >
        <div className="w-10 h-10 rounded-xl bg-crm-primary/20 flex items-center justify-center shrink-0">
          {notification.type === 'mention' ? (
            <AtSign size={18} className="text-crm-accent" />
          ) : (
            <MessageSquare size={18} className="text-crm-primary" />
          )}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold text-crm-text">{notification.sender?.name || 'New activity'}</p>
