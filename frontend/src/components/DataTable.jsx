            ...
            <tr className="bg-crm-darker/50 border-b border-crm-border">
              {columns.map((col, idx) => (
                <th 
                  ...
                  className={`px-3 py-3.5 text-xs font-semibold text-crm-text uppercase tracking-wider ${col.sortable !== false ? 'cursor-pointer hover:bg-crm-border/30' : ''}`}
              ...
              currentData.map((row, rowIdx) => (
                ...
                >
                  {columns.map((col, colIdx) => (
                    <td key={col.id || col.accessor || colIdx} className="px-3 py-3.5 text-sm whitespace-nowrap">
      ...
      {sortedData.length > itemsPerPage && (
        <div className="p-4 border-t border-crm-border flex items-center justify-between bg-crm-darker/30">
          <p className="text-sm text-crm-textMuted">
            Showing <span className="font-medium text-crm-text">{((currentPage - 1) * itemsPerPage) + 1}</span> to <span className="font-medium text-crm-text">{Math.min(currentPage * itemsPerPage, sortedData.length)}</span> of <span className="font-medium text-crm-text">{sortedData.length}</span> results
          </p>
          <div className="flex gap-2">
            <button
              onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
              disabled={currentPage === 1}
              className="p-2 rounded-lg bg-crm-card border border-crm-border text-crm-textMuted hover:text-crm-text disabled:opacity-50 transition-colors"
            >
              <ChevronLeft size={18} />
            </button>
            <button
