  ...
  try {
    ...
    const response = await axios.get(`${META_GRAPH_URL}/${formattedAdAccountId}/insights`, {
      params: {
        ...
        fields: 'campaign_id,campaign_name,spend,impressions,clicks,actions,action_values,outbound_clicks,inline_link_clicks,website_ctr',
        limit: 500,
...
const seedSimulatedData = async (clientId, days) => {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    if (metaAcc) {
      await metaAcc.update({ instagram_followers });
    }

    // Save daily snapshot to meta_account_metrics
    const metricExisting = await SaaSMetaAccountMetric.findOne({
      where: { client_id: clientId, date: dateStr }
    });
    if (metricExisting) {
      await metricExisting.update({ instagram_followers });
    } else {
...
const syncHistoricalAndLive = async (clientId, adAccountId, accessToken, days = 180) => {
  ...
  try {
    const igFollowerCount = await fetchInstagramFollowers(accessToken);
    if (igFollowerCount !== null) {
      const metaAcc = await SaaSMetaAccount.findOne({ where: { client_id: clientId } });
      if (metaAcc) {
        await metaAcc.update({ instagram_followers: igFollowerCount });
        console.log(`[META_SYNC] Updated account-level instagram_followers to ${igFollowerCount} for client ${clientId}`);
      }

      // Upsert daily snapshot in meta_account_metrics
      const todayStr = new Date().toISOString().split('T')[0];
      const metricExisting = await SaaSMetaAccountMetric.findOne({
        where: { client_id: clientId, date: todayStr }
      });
      if (metricExisting) {
        await metricExisting.update({ instagram_followers: igFollowerCount });
      } else {
        await SaaSMetaAccountMetric.create({
          client_id: clientId,
          instagram_followers: igFollowerCount,
          date: todayStr
        });
      }
    }
