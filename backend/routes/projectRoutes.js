...
const { protect, admin, checkWorkspaceAccess } = require('../middleware/authMiddleware');
const { 
...
router.get('/requests/:workspaceId', protect, checkWorkspaceAccess, getProjectRequests);
router.put('/requests/:id', protect, admin, updateRequestStatus);

// --- Existing Project Routes ---

// Get client/workspace project stats
router.get('/stats/:workspaceId', protect, checkWorkspaceAccess, async (req, res) => {
  try {
...
router.post('/', protect, admin, async (req, res) => {
  try {
    ...
    const projectData = { ...req.body, workspaceId: activeWorkspaceId };
