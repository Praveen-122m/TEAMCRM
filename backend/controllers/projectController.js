...
const createProjectRequest = async (req, res) => {
  try {
    const { title, description, budget, deadline, requiredFeatures, priority } = req.body;
...
const updateRequestStatus = async (req, res) => {
  try {
    ...
    if (request.workspaceId !== req.user.workspaceId && req.user.role !== 'SuperAdmin' && req.user.role !== 'super_admin') {
      return res.status(403).json({ message: 'Access denied: Request does not belong to your workspace' });
    }
