...
const formatLead = (lead) => {
  ...
  j.leadName = j.name;
  // Resolve clientName from various possible associations
  j.clientName = 
    j.client?.companyName || 
    j.client?.user?.name || 
    j.saasClient?.company_name || 
    j.saasClient?.client_name || 
    j.workspace?.saasClient?.company_name || 
    j.workspace?.saasClient?.client_name || 
    '—';
  j.campaignName = j.campaign?.name || j.campaignName || '—';
...
const getClientProfileId = async (userId) => {
  ...
  if (c) return c._id;
  const sc = await SaaSClient.findByPk(userId);
  if (sc) return sc.id;
  return null;
};
...
const buildLeadWhere = async (req, query) => {
  ...
  const { clientId, campaignId, status, source, search, dateFrom, dateTo } = query;

  if (req.user.role === 'Client') {
    const sc = await SaaSClient.findByPk(req.user._id);
    if (sc) {
      where.workspaceId = sc.workspace_id;
    } else {
    ...
    const sc = await SaaSClient.findByPk(clientId);
    if (sc) {
      if (sc.workspace_id.toString() !== activeWorkspaceId.toString()) {
        return { denied: true };
      }
    } else {
...
const leadIncludes = [
  ...
  },
  {
    ...
    as: 'saasClient',
    required: false,
    attributes: ['id', 'company_name', 'client_name', 'email'],
  },
  {
    model: Workspace,
    as: 'workspace',
    required: false,
    include: [{ model: SaaSClient, as: 'saasClient', required: false }]
  },
  { model: MetaAdsCampaign, as: 'campaign', required: false, attributes: ['_id', 'name'] },
  {
    model: Member,
    as: 'assignedMember',
    required: false,
